...
public class TetrisBoard implements Serializable{
    ...
    public int placementHeight(TetrisPiece piece, int x) {
        ...
        for(i = x; i < x + piece.getWidth(); i++) {
            temp = piece.getLowestYVals()[i-x]- colCounts[i];
            if(temp < max) {
                max = temp;
                result = i;
            }
        }
        int one = colCounts[result];
        int two = piece.getLowestYVals()[result-x];
        int height = one-two;
        return height;
        }
    ...
    public int placePiece(TetrisPiece piece, int x, int y) {
        ...
        for (int i = 0; i < piece.getBody().length; i++) {
            new_x = x + piece.getBody()[i].x;
            new_y = y + piece.getBody()[i].y;


            if ((new_x < 0 || new_x >= width) || (new_y < 0 || new_y >= height)) {
                return ADD_OUT_BOUNDS;
            } else if (tetrisGrid[new_x][new_y]) {
                return ADD_BAD;
            }
        }
        ...
        int neww_y;
        for (int i = 0; i < piece.getBody().length; i++) {
            neww_x = x + piece.getBody()[i].x;
            neww_y = y + piece.getBody()[i].y;

            tetrisGrid[neww_x][neww_y] = true;
            rowCounts[neww_y]++;

            if (neww_y + 1 > colCounts[neww_x]) {
                colCounts[neww_x] = neww_y + 1;
            }

            if (rowCounts[neww_y] >= width) {
                filled = true;
            }
    ...
    public int clearRows() {
        ...
        for(int i = 0; i < height; i++) {
            if(rowCounts[i] >= width) {
                ...
                clear = true;
                helper(i);
                System.arraycopy(rowCounts,i+1,rowCounts,i, height-1-i);
    ...
    private void helper(int i) {
        for(int j = 0; j < width; j++) {
            System.arraycopy(tetrisGrid[j],i+1,tetrisGrid[j],i,height-1-i);
